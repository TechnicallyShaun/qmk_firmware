...
#ifdef OLED_ENABLE
// Override the rotation to disable parent keyboard code
...
bool oled_task_user(void) {
    if (is_keyboard_master()) {
        // Primary side - show layer, key, and rate
        oled_write_P(PSTR("Layer: "), false);
        oled_write(get_u8_str(get_highest_layer(layer_state), ' '), false);
        oled_write_P(PSTR("  \n"), false);  // Clear rest of line

        oled_write_P(PSTR("Key: "), false);
        oled_write(custom_keycode_string(current_keycode), false);
        oled_write_P(PSTR("     \n"), false);  // Clear rest of line

        oled_write_P(PSTR("Rate: "), false);
        oled_write(get_u16_str(loop_rate, ' '), false);
        oled_write_P(PSTR("  \n"), false);  // Clear rest of line
    } else {
        // Secondary side - show WPM or other info
        oled_write_P(PSTR("Sofle Pro\n"), false);
        oled_write_P(PSTR("\nWPM: "), false);
        oled_write(get_u8_str(get_current_wpm(), ' '), false);
        oled_write_P(PSTR("  "), false);  // Clear rest
    }

    return false;  // Must return false to actually update display
}
#endif
